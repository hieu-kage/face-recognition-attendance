# --- CÁC THƯ VIỆN CƠ BẢN & XỬ LÝ DỮ LIỆU ---
import os
import math
import numpy as np
import pandas as pd
...
# --- TENSORFLOW & KERAS (QUAN TRỌNG CHO MODEL) ---
import tensorflow as tf
from tensorflow.keras.models import Model, load_model
from tensorflow.keras.layers import Input

# --- CÁC THƯ VIỆN CHUYÊN DỤNG CHO FACE RECOGNITION ---
# 1. Để load FaceNet gốc (cho phần Rebuild Model)
try:
...
# 2. Để tính khoảng cách (Euclidean Distance)
from sklearn.metrics.pairwise import euclidean_distances as L2

# 3. Để phát hiện khuôn mặt (cho Section 11 - Face Detection)
try:
...
# 9. REBUILD ARCHITECTURE AND LOAD WEIGHTS (THAY THẾ)
# =====================================================

print("\n=== Rebuilding Model & Loading Weights ===")

try:
    # BƯỚC 1: Xây dựng lại kiến trúc (Rebuild Architecture)
    # Chúng ta khởi tạo lại FaceNet gốc, cấu trúc y hệt lúc train
    print("1. Rebuilding model architecture...")
    from keras_facenet import FaceNet

    # Khởi tạo khung mạng FaceNet
    embedder_loader = FaceNet()
    rebuilt_model = embedder_loader.model

    # Đảm bảo input shape đúng chuẩn (160, 160, 3)
    # Bước này quan trọng để khớp với ma trận weights
    inputs = Input(shape=(160, 160, 3))
    outputs = rebuilt_model(inputs)
    embeddings = Model(inputs, outputs)

    # BƯỚC 2: Load file Weights
    # Đường dẫn file weights đã lưu ở Section 8 (cách 2)
    weights_path = "/kaggle/working/models/siamese_weights.weights.h5"

    if os.path.exists(weights_path):
        print(f"2. Loading weights from: {weights_path}")
        # Load weights vào model vừa dựng
...
# =====================================================
# 10. EVALUATION WITH REBUILT MODEL
# =====================================================

print("\n=== Evaluating Rebuilt Model ===")

# Generate embeddings using the REBUILT model
anchor_embeddings = []
positive_embeddings = []
negative_embeddings = []

print("Generating embeddings for test set...")
# Lưu ý: embeddings ở đây là model vừa được load weights
